...
coloc_analysis <- function(input_data, x = 'x', y = 'y', nbins = 2000, verbose = F, radius = 50){
...
coloc_analysis_replicates <- function(input_data, reps=1:length(input_data), x = 'x', y = 'y', nbins = 2000, verbose = F, radius = 50){
  
  celltypes <- colnames(input_data[[1]] %>% select(-x, -y))
  n <- length(celltypes)
...
## test run
outs <- coloc_analysis_replicates(
  res
)
